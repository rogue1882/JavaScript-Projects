...
function Handle_Operator(Next_Operator) {
    ...
    //displayed on the screen to a number and then store the resut in
    //Calculator.First_Operand if it doesn't already exist
    const Value_of_Input = parseFloat(Display_Value);
    //checks if an operator already exisits and if Wait_Second_Operand is true,
    ...
    } else if (operator) {// checks if an operator alreay exists
...
Update_Display(); 
    ...
    keys.addEventListener('click', (event)=> {
        ...
        // ensures that AC clears all the numbers from the Calculator
